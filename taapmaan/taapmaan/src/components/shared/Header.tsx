...
const Header: React.FC<HeaderProps> = ({ city, level, temp, dataSource, updatedAt, onMenuClick }) => {
  ...
  return (
    <header className="sticky top-0 z-50 p-6 animate-in fade-in slide-in-from-top-4 duration-700">
      <div className="bg-white/80 backdrop-blur-2xl rounded-none flex items-center justify-between px-6 py-5 shadow-xl shadow-slate-200/50 border border-white">
        <div className="flex items-center gap-5">
          <button 
            onClick={onMenuClick}
            className="w-12 h-12 flex items-center justify-center bg-slate-50 rounded-none transition-all active:scale-90 border border-slate-100"
          >
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              ...
              <div className={`px-2 py-0.5 rounded-none text-[7px] font-black uppercase tracking-widest border border-slate-100 bg-slate-50 text-slate-400`}>
        ...
        <div className="flex items-center gap-3">
            ...
            <button 
                onClick={handleShare}
                className="w-10 h-10 flex items-center justify-center bg-slate-900 rounded-none text-white shadow-lg shadow-slate-900/20 active:scale-95"
            >
                <Share2 size={16} />
            </button>
        </div>
