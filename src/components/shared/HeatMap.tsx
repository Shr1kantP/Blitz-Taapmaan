...
const HeatMap: React.FC<HeatMapProps> = ({
  ...
  const setupMap = (element: HTMLDivElement, isModal: boolean) => {
    ...
    // Also add the user's current center
    const currentWeight = getWeightAt(centerLat, centerLon, intensity);
    points.push([centerLat, centerLon, currentWeight]);

    // Add local jittered points around the current center for "every corner" feel
    for (let i = 0; i < 5; i++) {
    ...
    // Heatmap layer for "atmospheric" effect - adjusted to "shapes"
    const heatmap = L.heatLayer(points, {
      radius: isModal ? 60 : 45, // Larger radius for "shapes"
      blur: isModal ? 40 : 35,   // More atmospheric blur
      maxZoom: 13,
  ...
  useEffect(() => {
    ...
    // Inject pulse animation CSS (Removed as per request)
    /*
